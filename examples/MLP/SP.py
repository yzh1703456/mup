        ...
        for criterion in [F.cross_entropy]:
            for width in [64, 128, 256, 512, 1024, 2048, 4096, 8192]:
                for sample in range(sample_num):
                    ...
                    # save model
                    if not os.path.exists(f'sp_model_{width}'):
                        os.makedirs(f'sp_model_{width}')
                    torch.save(mynet, f'sp_model_{width}/mlp_width_{width}_lr_{lr}.pth')
                    json_data = json.dumps(log_list)
                    # 将 json 数据写入文件
                    with open("log_sp.json", "w", encoding='utf-8') as file:
                        file.write(json_data)



    with open(os.path.join(os.path.expanduser(args.log_dir), 'sp_logs.tsv'), 'w') as f:
        logdf = pd.DataFrame(logs)
        print(os.path.join(os.path.expanduser(args.log_dir), 'sp_logs.tsv'))
