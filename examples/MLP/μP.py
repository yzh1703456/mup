                ...
                for sample in range(sample_num):
                    ...
                    if args.save_base_shapes:
                        ...
                        if args.deferred_init:
                            ...
                            base_shapes = get_shapes(MLP(width=width, nonlin=nonlin, output_mult=args.output_mult,
                                                         input_mult=args.input_mult))
                            delta_shapes = get_shapes(
                                # just need to change whatever dimension(s) we are scaling
                        ...
                        sys.exit()
                    mynet = MLP(width=width, nonlin=nonlin, output_mult=args.output_mult,
                                input_mult=args.input_mult).to(device)
                    if args.load_base_shapes:
                        print(f'loading base shapes from width{width}.bsh')
                        set_base_shapes(mynet, f'width{width}.bsh')
                        print('done')
                    else:
                        print(f'using own shapes')
                        set_base_shapes(mynet, None)
                        print('done')
                    optimizer = MuSGD(mynet.parameters(), lr=lr, momentum=args.momentum)
                    for epoch in range(1, 20+1):
                        ...
                        if math.isnan(train_loss):
                            break
                    test_loss, test_acc = test(args, mynet, device, test_loader)
                    train_loss, train_acc = test(args, mynet, device, train_loader)
                    log_item = {"width": width, "lr": lr, "sample": sample, "train_loss": train_loss,
                                "train_acc": train_acc, "test_loss": test_loss, "test_acc": test_acc}
                    log_list.append(log_item)
                    if not os.path.exists(f'miup_model_{width}'):
                        os.makedirs(f'miup_model_{width}')
                    torch.save(mynet, f'miup_model_{width}/mlp_width_{width}_lr_{lr}.pth')
                    json_data = json.dumps(log_list)
                    # 将 json 数据写入文件
                    with open("log_miup.json", "w", encoding='utf-8') as file:
                        file.write(json_data)



    with open(os.path.join(os.path.expanduser(args.log_dir), 'miup_logs.tsv'), 'w') as f:
        logdf = pd.DataFrame(logs)
        print(os.path.join(os.path.expanduser(args.log_dir), 'miup_logs.tsv'))
